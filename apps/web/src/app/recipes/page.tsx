...
const DEFAULT_FILTERS: Filters = { category: '', tag: '', healthTag: '', maxCalories: 1500, maxPrepTime: 120, maxCost: 30 };
...
function RecipesContent() {
  ...
  useEffect(() => {
    ...
    setRecipes([]);
    fetchRecipes(1, true);
  }, [search, filters.category, filters.tag, filters.healthTag, filters.maxCalories, filters.maxPrepTime, filters.maxCost]);

  async function fetchRecipes(p: number, reset = false) {
    setLoading(true);
    try {
      const pageSize = (filters.tag || filters.healthTag) ? 100 : 12;
      ...
      let filtered = data;
      if (filters.tag) {
        filtered = filtered.filter((r: any) => r.tags?.some((t: string) => t.toLowerCase().includes(filters.tag.toLowerCase())));
      }
      if (filters.healthTag) {
        filtered = filtered.filter((r: any) => r.healthTags?.some((t: string) => t.toLowerCase().includes(filters.healthTag.toLowerCase())));
      }
      const sorted = filtered.sort((a: any, b: any) => a.title.localeCompare(b.title, 'de'));
      setRecipes(prev => reset ? sorted : [...prev, ...sorted]);
      setHasMore(!filters.tag && !filters.healthTag && data.length === 12);
    } catch {
      // API nicht verfuegbar
    } finally {
      setLoading(false);
    }
  }
