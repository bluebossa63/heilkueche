    ...
    <>
      ...
      <Link href={`/recipes/detail?id=${recipe.id}`} className="group block">
        <div className="card overflow-hidden h-full flex flex-col group-hover:shadow-card-hover transition-shadow">
          {/* Image */}
          <div className="relative h-48 bg-gradient-to-br from-sage to-sage-dark flex-shrink-0">
            {recipe.imageUrl ? (
            ...
            {/* Category chip */}
            <span className="absolute top-3 left-3 bg-white/90 backdrop-blur text-xs font-semibold px-2.5 py-1 rounded-full text-primary-dark">
              {CATEGORY_DE[recipe.category] || recipe.category}
            </span>
            {/* Health tags */}
            {recipe.healthTags && recipe.healthTags.length > 0 && (
              <div className="absolute bottom-3 left-3 flex gap-1">
                {recipe.healthTags.slice(0, 4).map(tag => (
                  <span key={tag} className="bg-white/90 backdrop-blur text-sm rounded-full w-7 h-7 flex items-center justify-center" title={tag}>
                    {HEALTH_TAG_LABELS[tag] || tag.charAt(0)}
                  </span>
                ))}
              </div>
            )}
            ...
            >
              <span className={saved ? 'text-primary-dark' : 'text-charcoal-light'}>&#9829;</span>
            </button>
          </div>
                            ...
                            <div className="flex gap-1">
                              {(['breakfast', 'lunch', 'dinner'] as const).map(slot => (
                                <button
                                  ...
                                  className="text-[10px] px-2 py-1 rounded-lg bg-sage hover:bg-primary/10 transition-colors flex-1"
                                >
                                  {SLOT_DE[slot]}
                                </button>
                              ))}
