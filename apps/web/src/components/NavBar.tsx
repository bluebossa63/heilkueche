...
const navLinks = [
  { href: '/conditions',    label: 'Gesundheitsthemen' },
...
export function NavBar() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-cream/90 backdrop-blur border-b border-primary/10">
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6">
          {navLinks.map(({ href, label }) => (
            <Link key={href} href={href} className="text-sm font-medium text-charcoal-light hover:text-primary transition-colors">
              {label}
            </Link>
          ))}
          {isAuthenticated ? (
        ...
        <button className="md:hidden p-2 rounded-xl hover:bg-sage-dark transition-colors" onClick={() => setOpen(!open)} aria-label="Menu umschalten">
      ...
      {open && (
        <div className="md:hidden border-t border-primary/10 bg-cream px-4 py-4 flex flex-col gap-3">
          {navLinks.map(({ href, label }) => (
